...
void SampleMonoBehaviour::Update()
{
	...
	Camera::get_main()->get_transform()->position.Get().x += EngineTime::GetDeltaTime();

	fpsText->GetComponent<TextLabel>()->SetText("fps: "s + std::to_string(1 / EngineTime::GetDeltaTime()));
}